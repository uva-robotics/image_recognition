...
if __name__ == '__main__':
  ...
  rp = RosPack()
  path = rp.get_path('image_recognition_tensorflow')
  ...
  if not FLAGS.bottleneck_dir:
      bottleneck_dir = os.path.join(path, "bottlenecks")
      if not os.path.exists(bottleneck_dir):
          os.makedirs(bottleneck_dir)
      FLAGS.bottleneck_dir = bottleneck_dir

  if not FLAGS.summaries_dir:
      summaries_dir = os.path.join(path, "summaries_dir", FLAGS.architecture)
      if not os.path.exists(summaries_dir):
          os.makedirs(summaries_dir)
      FLAGS.summaries_dir = summaries_dir

  if not FLAGS.output_labels:
      output_labels = os.path.join(path, "output_labels.txt")
      FLAGS.output_labels = output_labels

  if not FLAGS.output_graph:
      output_graph = os.path.join(path, "output_graph.pb")
